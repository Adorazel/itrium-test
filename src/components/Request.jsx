...
const Request = props => {
  ...
  const {
    reqError, resError, fetchError, url, method, headers, body,
    urlChangeHandler, methodChangeHandler, headersChangeHandler, bodyChangeHandler,
    sendRequestHandler, keyDownHandler, clearHeaderHandler, addHeaderHandler
  } = props
  ...
  return <section ref={section} className="border border-left-0 border-right-0 border-top-0 mb-3 pb-2">
    <div className="d-flex mb-3">
      <h5 className="text-uppercase m-0 mr-2">Request</h5>
      <QueueContainer/>
    </div>
    ...
    <div className="input-group mb-3">
      <div className="input-group-prepend">
        <select className="custom-select rounded-left rounded-right-0"
                value={method} onChange={methodChangeHandler}>
    ...
    <div className="tab-content">
      <div className="tab-pane pt-3 active" id="body" role="tabpanel" aria-labelledby="body-tab">
        <div className="form-group">
          ...
          <Editor name="response-editor" value={body} onChange={bodyChangeHandler}/>
        </div>
      </div>
      <div className="tab-pane py-3" id="headers" role="tabpanel" aria-labelledby="headers-tab">
        ...
        <div className="text-right">
          <button type="button" className="btn btn-success btn-sn"
                  onClick={event => addHeaderHandler(event)}>
            ADD HEADER
          </button>
        </div>
      </div>
